...
CONT=`ckyorn -Q -d yes -p "Do you want to Split this VMail box? (Default Yes)?"` || exit $?
        case $CONT in
        [yY]*)  ;;
        *)
        echo "VMail box will be Split."
        sed -e "s/Join/Split/" temp/$F > TMP_$F;mv TMP_$F temp/$F
                ;;
        esac
